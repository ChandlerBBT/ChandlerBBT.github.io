...
summary: "一份面向部署人员和普通员工的部门 AI 助理手册：机器采购建议、Hermes 安装、30 人批量开通、飞书机器人接入、部门知识库、权限隔离、token 统计、业务截图案例和用户上手流程。"
tags: ["公司端 AI", "飞书", "小龙虾", "Hermes", "DeepSeek", "知识库", "Mac Studio"]
cover: "/assets/img/posts/feishu-lobster-deployment/case-start.png"
draft: false
changelog: ["0.4.0 - 补充部门知识库、开源模型兜底方案、机器配置档位和悬浮目录。", "0.3.0 - 改为部署手册和用户手册合一版本，补充 Hermes 安装、批量开通、飞书上手流程、业务案例截图和图片放大交互。", "0.2.0 - 重写为正式部署规划，补充飞书端用户流程、后端服务、自动化开通、目录/端口/配置、权限和运维细节。", "0.1.0 - 第一版，确定飞书主入口和一人一 Hermes profile/gateway 的方向。"]
---
...
```bash
sudo -iu lobster
mkdir -p /Users/Shared/lobster/department-kb/{raw,extracted,chunks,vectors,reports,audit}
chmod -R 750 /Users/Shared/lobster/department-kb
```
...
资料入库命令建议设计成两步，先 dry-run，再正式写入：

```bash
lobsterctl kb import \
  --source "/Volumes/ResearchArchive/2025-2026" \
  --data-class internal-kb \
  --project "department-research-archive" \
  --allowed-roles "user,pilot,maintainer,admin" \
  --dry-run
...
第一版建议用 `lobsterctl kb rebuild --since 2026-01-01` 做增量重建，不要每次全量重跑。每个回答必须返回引用来源，至少包含文档标题、页码或章节、导入批次和片段 ID。

本地模型 server 只在内网 API 不可用时启动：

```bash
llama-server \
  --host 127.0.0.1 \
  --port 8090 \
  --model /Users/Shared/lobster/models/qwen3-30b-a3b-instruct-2507-q4.gguf \
  --ctx-size 32768 \
  --threads 10
```

如果机器是 64GB，`--ctx-size` 先从 32768 起步；如果机器是 96GB，可以逐步测试 65536。不要为了长上下文牺牲飞书 Router、Hermes gateway 和索引任务的稳定性。
...
```text
/dept-kb search 关键词
/dept-kb ask 过去 6 个月用户为什么放弃试用？
/dept-kb sources 回答ID
/dept-kb save 当前回答到个人知识库
```

![部门知识库业务截图：员工通过 /dept-kb ask 检索授权部门资料，回答附带来源和后续操作按钮，示例数据用于培训演示。](/assets/img/posts/feishu-lobster-deployment/case-dept-kb.png)

需要形成新课题输入时，用桌面研究命令：

```text
/desk-research 新课题：AI 助理如何提升研究团队效率
范围：部门研究报告 + 用户调研材料
输出：1 页 briefing，包含结论、证据、风险和建议动作
```

![桌面研究业务截图：员工通过 /desk-research 把多份部门材料合成为 briefing 草稿，示例数据用于培训演示。](/assets/img/posts/feishu-lobster-deployment/case-desk-research.png)
...
```text
/dept-kb ask 请基于 2025 年以来的用户调研材料总结：
1. 用户首次试用失败的前三个原因
2. 每个原因对应的证据来源
3. 可以在下次访谈里继续验证的问题
```
...
```text
/desk-research 新课题：行业客户为什么需要部门级 AI 助理
范围：部门研究报告、用户访谈、售前反馈
输出：
1. 一页 briefing
2. 3 条关键洞察
...
10. Department KB Searcher：按权限检索部门资料，并强制返回引用来源。
11. Desk Research Composer：把多份授权材料合成为 briefing 或桌面研究草稿。
12. Skill Discovery Log：记录候选 skills，不直接全员启用。
