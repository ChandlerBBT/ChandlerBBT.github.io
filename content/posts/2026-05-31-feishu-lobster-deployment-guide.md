...
summary: "一份面向部署人员和普通员工的部门 AI 助理手册：机器采购建议、Hermes 安装、Excel 批量开通、飞书接入联调、部门知识库素材上传、权限隔离、token 统计、业务截图案例和用户上手流程。"
...
draft: false
changelog: ["0.5.0 - 补充飞书接入联调、Excel 批量开通、知识库上传向导、用户飞书截图路径和技能库分层。", "0.4.0 - 补充部门知识库、开源模型兜底方案、机器配置档位和悬浮目录。", "0.3.0 - 改为部署手册和用户手册合一版本，补充 Hermes 安装、批量开通、飞书上手流程、业务案例截图和图片放大交互。", "0.2.0 - 重写为正式部署规划，补充飞书端用户流程、后端服务、自动化开通、目录/端口/配置、权限和运维细节。", "0.1.0 - 第一版，确定飞书主入口和一人一 Hermes profile/gateway 的方向。"]
---
...
6. 普通员工只使用飞书；部署者使用管理台完成开通、联调、知识库上传，命令行只作为底层排障手段。
...
第一版建议给部署者提供一个本机管理台，地址只允许内网访问，例如 `https://lobster.company.com/admin`。部署者日常只需要走五个向导：
...
这条路径的目标是让部署人员尽量少碰底层命令。下面的命令和配置用于首次安装、自动化脚本开发和排障；正式上线后，开通员工、接入飞书和导入知识库都应该在管理台里完成。
...
部署者在管理台里看到的是模型配置向导。选择“公司内网大模型 API”后，只需要填写 API Base、API Key、默认模型名和最大上下文；选择“本机离线模型”后，只需要选择已下载模型和上下文长度。保存前必须点“测试连接”，测试通过后才允许导入部门知识库素材。

![模型配置向导截图：部署者选择公司内网 API 或本机模型，测试通过后保存为知识库默认模型。](/assets/img/posts/feishu-lobster-deployment/admin-model-config.png)

### 5. 用 Excel 准备员工开通表

部署者只准备一份 Excel，不需要填写端口、open_id、profile 名称或系统路径。第一版必填四列：
...
Excel 导入通过后，管理台会生成内部 `users.csv` 和 profile registry。内部表会补齐 `feishu_open_id`、`employee_id`、`gateway_port`、`profile_name`、`quota_gb` 和 `monthly_token_budget` 等字段，但这些字段不要求部署者手工维护。
...
飞书接入分两边配置：飞书开放平台创建应用，部署管理台保存应用密钥和回调参数。部署人员应该先在管理台打开“飞书接入”向导，再去飞书开放平台操作。
...
![飞书接入向导截图：部署者把飞书应用参数填入管理台，并复制事件回调地址到飞书开放平台。](/assets/img/posts/feishu-lobster-deployment/admin-feishu-wizard.png)

飞书开放平台侧按下面顺序配置：

1. 进入飞书开放平台，创建“企业自建应用”，应用名建议为“部门小龙虾”。
2. 启用机器人能力，头像和简介使用部门内部统一口径。
3. 在事件订阅里填写管理台给出的事件回调地址，保存后飞书会发送 challenge，管理台应显示“challenge 通过”。
4. 订阅消息事件：私聊消息、群聊 @ 消息、文件消息。
5. 订阅卡片按钮回调：保存到知识库、生成桌面研究、查看引用、确认资料类型。
6. 开通发送消息能力：机器人发送文本、富文本、交互卡片。
7. 开通通讯录读取能力：用员工工号匹配飞书 open_id。只读取开通所需字段，不同步无关人事信息。
8. 发布应用版本，并安装到部门可见范围。第一轮先只安装给 3 到 5 个试点用户。
...
调通标准不是“配置保存成功”，而是下面 6 项全部通过：飞书 challenge、私聊消息事件、机器人发送消息、卡片按钮回调、工号到 open_id 的映射、个人 profile 路由。
...
技能库按三条线维护：效率工具、产品开发、自媒体。部署完成后先全员启用低风险效率类；涉及浏览器登录态、网页自动化、外部账号和内容发布的技能，需要申请后按角色开放。

![技能库配置截图：三条线 11 个优质 skills，按岗位启用并做审计。](/assets/img/posts/feishu-lobster-deployment/admin-skill-pack.png)

第一类，效率工具：

1. Agent Reach：公开信息检索、RSS、社媒和网页资料追踪，默认只允许公开来源。
2. NotebookLM Skill：把授权资料整理成可追问的笔记本，适合研究资料复盘。
3. bb-browser：带登录态浏览器自动化，默认不全员启用，只给通过审批的岗位。
4. skill-creator：把稳定流程沉淀成可复用 skill，默认由维护者使用。

第二类，产品开发：

1. superpowers：把需求拆成明确工作流，适合复杂任务前置规划。
2. frontend-design：产品原型、交互草图、前端视觉方案，面向产品和研发试点用户。
3. ui-ux-pro-max：UI/UX 评审、信息架构、可用性检查，适合产品评审前使用。
4. Humanizer-zh：优化中文表达，减少生硬的 AI 味，适合周报、方案和对外文案。

第三类，自媒体和表达：

1. find-skills：帮助员工发现更合适的 skill，作为技能库入口。
2. office-hours：用于商业想法、市场判断、产品策略的结构化质询。
3. Playwright MCP：网页交互验证和截图，默认只给研发、测试和维护者。
...
5. Department KB Searcher：按权限检索部门资料，并强制返回引用来源。
6. Desk Research Composer：把多份授权材料合成为 briefing 或桌面研究草稿。
