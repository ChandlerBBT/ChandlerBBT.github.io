...
from analyzing_financial_data_r_common import CACHE_NAME, SLUG, extract_page_text, is_omittable_pdf_page, read_json, write_json
...
class RCodeParser(HTMLParser):
    def __init__(self) -> None:
        super().__init__(convert_charrefs=True)
        self.in_r_code = False
        self.current: list[str] = []
        self.blocks: list[str] = []
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
        ...
        values = {name.lower(): value or "" for name, value in attrs}
        classes = values.get("class", "")
        if "language-r" in classes.split():
            self.in_r_code = True
            self.current = []

    def handle_endtag(self, tag: str) -> None:
        if tag == "code" and self.in_r_code:
            self.blocks.append("".join(self.current))
            self.current = []
            self.in_r_code = False
...
def r_code_blocks(raw: str) -> list[str]:
    parser = RCodeParser()
    parser.feed(raw)
    return parser.blocks


def count_forbidden_python_code_blocks(raw: str) -> int:
    return len(python_code_blocks(raw))
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Audit the Ang financial data and R modeling Chinese translation output.")
    ...
    forbidden_python_code: list[tuple[Path, str]] = []
    forbidden_code_tabs: list[tuple[Path, str]] = []
    ...
    for audit in audits:
        ...
        for pattern in SYMBOL_ARTIFACT_PATTERNS_COMPILED:
            ...
            if match:
                ...
                break
        if count_forbidden_python_code_blocks(raw):
            forbidden_python_code.append((audit.path, str(count_forbidden_python_code_blocks(raw))))
        if 'class="code-tabs"' in raw:
            forbidden_code_tabs.append((audit.path, "code-tabs"))
    ...
    report = {
        ...
        "forbidden_code_tab_blocks": sum(raw.count('class="code-tabs"') for raw in raw_pages),
        "forbidden_python_code_blocks": sum(count_forbidden_python_code_blocks(raw) for raw in raw_pages),
    ...
    print("analyzing financial data R translation quality report")
    ...
    for label, items in [
        ...
        ("Forbidden Python code block", forbidden_python_code[:10]),
        ("Forbidden code tab", forbidden_code_tabs[:10]),
    ]:
