...
CACHE_NAME = "analyzing-financial-data-r"
...
R_CODE_PLACEHOLDER_RE = re.compile(r"\[\[R_CODE:([A-Za-z0-9_.:-]+)\]\]")
...
def unit_kind(title: str) -> str:
    ...
    if lower == "index" or APPENDIX_RE.match(cleaned):
        return "back_matter"
    if lower.startswith("preface") or lower in {"notation", "acknowledgments", "dedication"}:
        return "front_matter"
...
def render_r_code_block(code_id: str, r_code: str) -> str:
    safe_id = re.sub(r"[^A-Za-z0-9_-]+", "-", code_id).strip("-") or "code"
    return f"""
<div class="r-code-block" id="{html.escape(safe_id, quote=True)}">
  <pre><code class="language-r">{html.escape(r_code.rstrip())}</code></pre>
</div>""".strip()


def replace_r_code_placeholders(fragment: str, code_blocks: list[dict[str, Any]]) -> str:
    by_id = {
        str(item.get("id", "")).strip(): render_r_code_block(
    ...
    return R_CODE_PLACEHOLDER_RE.sub(replace, fragment)
